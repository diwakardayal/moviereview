...
const EmailVerfication = () => {
	...
	return (
		<div className="flex justify-center items-center h-100 w-full h-screen text-white bg-primary">
			<form className="bg-secondary px-5 py-4 rounded-md">
				...
				<div className="flex flex-col space-y-2 mt-6">
					...
					<input
						id="email"
						className="p-1  rounded bg-secondary border-2 border-dark-subtle text-white"
				...
				<div className="flex flex-wrap gap-2 justify-around mt-5 items-center">
					{otp.map((_, index) => (
						<input
							...
							type="number"
							key={index}
							className="spin-button-none h-12 w-12 border-2 rounded text-white bg-secondary p-4 font-semibold text-xl"
						/>
					))}
				</div>
